...
impl TokenAccount {
    pub fn get_amount(account: &AccountInfo) -> Result<u64, ProgramError> {
        let token_account = state::TokenAccount::from_account_info(account)?;
...
pub struct MintAccount2022Account;

impl AccountCheck for MintAccount2022Account {
    fn check(account: &AccountInfo) -> Result<(), ProgramError> {
        if !account.is_owned_by(&TOKEN_2022_PROGRAM_ID) {
            return Err(PinocchioError::InvalidOwner.into());
        }
...
impl AccountInit for MintAccount2022Account {
    fn init(
        account: &AccountInfo,
        mint: &&AccountInfo,
        payer: &AccountInfo,
